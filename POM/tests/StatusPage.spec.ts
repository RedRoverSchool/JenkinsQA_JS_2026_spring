...
test.describe("US_02.002 | Freestyle Project Configuration > Project Description", () => {
  test("RF_02.002.10 | Verify description can be edited from Status page", async ({
    ...
    await app.statusPage.fillDescription(description);
    await app.statusPage.clickSaveButton();

    await app.statusPage.clickAddDescriptionBtn();
    await app.statusPage.fillDescription(updatedDescription);
    await app.statusPage.clickSaveButton();

    await expect(app.statusPage.descriptionContent()).toContainText(
      updatedDescription,
    );
  });

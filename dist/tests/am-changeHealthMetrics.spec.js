...
test.describe("US_06.009| Multibranch pipeline Configuration > Change Health metrics", () => {
    ...
    test("TC_06.009.01 | Health Metrics section is displayed", async ({ page }) => {
    ...
    test("TC_06.009.02 | Verify that Add Metrics button is visible and provides 3 options", async ({ page }) => {
        await page.locator(".task a[href*='configure']").click();
    ...
    test("TC_06.009.03 | Each metric displays its specific fields after being added to Health Metrics section", async ({ page }) => {
        await page.locator(".task a[href*='configure']").click();
    ...
    test("TC_06.009.04 | 'Add Metrics' button shows only remaining options after metric is selected.", async ({ page }) => {
        await page.locator(".task a[href*='configure']").click();
        await navigateToHealthMetrics(page);
        await chooseTheMetric(page, "Child item with the given name");
    ...
    test("TC_06.009.05 | Metric is removed from the Health Metrics sections after clicking 'Remove' button", async ({ page }) => {
        await page.locator(".task a[href*='configure']").click();
        await navigateToHealthMetrics(page);
        await chooseTheMetric(page, "Child item with the given name");
...
//# sourceMappingURL=am-changeHealthMetrics.spec.js.map
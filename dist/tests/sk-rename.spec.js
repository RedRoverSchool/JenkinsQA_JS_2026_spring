...
test.describe("US_06.002 | Multibranch pipeline Configuration > Rename", () => {
    ...
    test("TC_06.002.02 | Renaming project from the main page", async ({ page }) => {
    ...
    test("TC_06.002.03 | Error when renaming a project with an empty name", async ({ page }) => {
        await page.getByRole("button", { name: "Save" }).click();
        await page.getByRole("link", { name: "Rename" }).click();
    ...
    jenkinsData.unsafeCharacters.forEach(unsafeCharacter => {
        test(`TC_06.002.04 | Error when renaming a project using a ${unsafeCharacter} character`, async ({ page }) => {
            await page.getByRole("button", { name: "Save" }).click();
            await page.getByRole("link", { name: "Rename" }).click();
            const newNameField = page.locator("[name='newName']");
            await newNameField.clear();
    ...
    test("TC_06.002.06 | Error when renaming a project with the existing name", async ({ page }) => {
        await page.getByRole("button", { name: "Save" }).click();
        await page.getByRole("link", { name: "Rename" }).click();
...
//# sourceMappingURL=sk-rename.spec.js.map
...
test.describe("US_05.001 | Folder Configuration > Display Name and Description", () => {
    test("TC_05.001.03 | Verify Configure page opens", async ({ page }) => {
        await createFolder(page, folderConfigData.folderName);
        await page.locator(folderConfigLocators.configureLink).click();
        await expect(page).toHaveURL(/configure/);
    });
    test("TC_05.001.04 | Verify Display Name and Description fields are available", async ({ page }) => {
        await createFolder(page, folderConfigData.folderName);
        await page.locator(folderConfigLocators.configureLink).click();
        await expect(page.getByLabel("Display Name")).toBeVisible();
        await expect(page.getByText("Description")).toBeVisible();
    });
    test("TC_05.001.05 | Verify user can update Display Name", async ({ page }) => {
        await createFolder(page, folderConfigData.folderName);
        await page.locator(folderConfigLocators.configureLink).click();
    ...
    test("TC_05.001.06 | Verify user can update Description", async ({ page }) => {
        await createFolder(page, folderConfigData.folderName);
    ...
    test("TC_05.001.07 | Verify empty Display Name and Description can be saved", async ({ page }) => {
        await createFolder(page, folderConfigData.folderName);
        await page.goto(`/job/${folderConfigData.folderName}/configure`);
    ...
    test("TC_05.001.08 | Verify Apply saves changes without leaving page", async ({ page }) => {
...
test.describe("US_05.004 | Folder Configuration > Save or Apply", () => {
    test("TC_05.004.01 | Verify Save and Apply buttons are displayed", async ({ page }) => {
    ...
    test("TC_05.004.02 | Verify Save persists changes and redirects to Folder main page", async ({ page }) => {
        const description = generateDescription();
        await createFolder(page);
        await page.getByRole("link", { name: "Configure" }).click();
        await page.locator("textarea").fill(description);
    ...
    test("TC_05.004.03 | Verify Apply persists changes without redirecting", async ({ page }) => {
        const description = generateDescription();
        await createFolder(page);
        await page.getByRole("link", { name: "Configure" }).click();
        await page.locator("textarea").fill(description);
...
//# sourceMappingURL=tl-folderConfiguration.spec.js.map
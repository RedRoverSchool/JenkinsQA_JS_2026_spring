...
test.describe("US_02.002 | Freestyle Project Configuration > Project Description", () => {
    ...
    test("TC_02.002.01 | Verify Description field is displayed", async ({ page }) => {
        const sectionTitle = page.locator("#general");
        await expect(sectionTitle).toBeVisible();
        const descriptionTextarea = page.locator("textarea[name='description']");
        await expect(descriptionTextarea).toBeVisible();
    });
    test("TC_02.002.02 | Verify Preview option is displayed", async ({ page }) => {
        const sectionTitle = page.locator("#general");
        await expect(sectionTitle).toBeVisible();
        const previewLink = page.locator("a.textarea-show-preview");
        await expect(previewLink).toBeVisible();
    });
    test("TC_02.002.03 | Add description via Configuration page", async ({ page }) => {
    ...
    test("TC_02.002.04 | Add description via Status page", async ({ page }) => {
    ...
    test("TC_02.002.05 | Edit description via Status page", async ({ page }) => {
        const descriptionInputField = page.locator("textarea[name='description']");
        await descriptionInputField.fill(description);
        await page.getByRole('button', { name: 'Save' }).click();
...
//# sourceMappingURL=ok-verifyFreestyleProjectDescription.spec.js.map
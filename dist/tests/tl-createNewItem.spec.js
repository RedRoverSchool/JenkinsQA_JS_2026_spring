...
test.describe("US_01.001 | New Item > Create a new item", () => {
    test("TC_01.001.21 | Verify new item page opens from dashboard", async ({ page }) => {
        await openNewItemPage(page);
        await expect(page).toHaveURL(/newJob/);
    });
    test("TC_01.001.22 | Verify blank item name validation", async ({ page }) => {
        await openNewItemPage(page);
    ...
    test("TC_01.001.24 | Verify successful item creation", async ({ page }) => {
        await openNewItemPage(page);
    ...
    test("TC_01.001.25 | Verify warning for duplicate item name", async ({ page }) => {
    ...
    test("TC_01.001.26 | Verify created item is displayed on dashboard", async ({ page }) => {
...
//# sourceMappingURL=tl-createNewItem.spec.js.map
...
test.describe("US_10.004 | Manage Jenkins > Tools", () => {
    ...
    test("TC_10.004.01 | Verify Tools Configuration Page opens", async ({ page }) => {
    ...
    for (const section of vvData.toolsSections) {
        test(`TC_10.004.02 | Verify Configuration section '${section}' is displayed`, async ({ page }) => {
            await expect(page.locator(".jenkins-section__title").filter({ hasText: section })).toBeVisible();
        });
    }
    test("TC_10.004.03 | Verify Maven settings provider can be changed to custom", async ({ page }) => {
    ...
    test("TC_10.004.04 | Verify Maven global settings provider can be changed to custom", async ({ page }) => {
...
//# sourceMappingURL=vv-toolsConfiguration.spec.js.map
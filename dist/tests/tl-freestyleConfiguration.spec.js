...
test.describe("US_02.002 | Freestyle Project Configuration > Project Description", () => {
    test("TC_02.002.06 | Verify Description field is displayed", async ({ page }) => {
        await createFreestyleProject(page, newItemData.freestyleProjectName);
        await page.locator("a[href$='/configure']").click();
        await expect(page.getByText("Description")).toBeVisible();
    });
    test("TC_02.002.07 | Verify Preview option is available", async ({ page }) => {
        await createFreestyleProject(page, newItemData.freestyleProjectName);
        await page.locator("a[href$='/configure']").click();
        await expect(page.getByRole("link", { name: "Preview" })).toBeVisible();
    });
    test("TC_02.002.08 | Verify description is displayed after Save", async ({ page }) => {
        await createFreestyleProject(page, newItemData.freestyleProjectName);
...
test.describe("US_02.003 | Freestyle Project Configuration > Configure SCM", () => {
    test("TC_02.003.02 | Verify Source Code Management section is available", async ({ page }) => {
        await createFreestyleProject(page);
        await page.getByRole("link", { name: "Configure" }).click();
        await expect(page.locator("#source-code-management")).toBeVisible();
    });
    test("TC_02.002.09 | Verify description can be added from Status page", async ({ page }) => {
        await createFreestyleProject(page);
...
//# sourceMappingURL=tl-freestyleConfiguration.spec.js.map
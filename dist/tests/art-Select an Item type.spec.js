...
test.describe("US_01.004 | New Item > Select an Item type", () => {
    test("TC_01.004.10 | Create new Pipeline", async ({ page }) => {
    ...
    test("TC_01.004.11 | FreeStyleProject", async ({ page }) => {
        await page.locator("#side-panel a[href$='newJob']").click();
    ...
    test("TC_01.004.12 | Multi-configuration project", async ({ page }) => {
        await page.locator("#side-panel a[href$='newJob']").click();
        await page.locator("#name").fill(jenkinsData.multiConfigProject);
        await page.locator(".hudson_matrix_MatrixProject").click();
        await page.locator("#ok-button").click();
        await page.locator(".app-jenkins-logo").click();
        const job = await page.locator("#projectstatus .jenkins-table__link").getAttribute("href");
        expect(job).toContain(jenkinsData.multiConfigProject);
    });
});
//# sourceMappingURL=art-Select%20an%20Item%20type.spec.js.map
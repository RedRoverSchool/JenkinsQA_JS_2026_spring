...
test.describe("US_01.005 | New Item > Multibranch Pipeline ", () => {
    test("TC_01.005.03 | Verify Multibranch Pipeline type is displayed", async ({ page }) => {
        await openNewItemPage(page);
        await expect(page.getByText("Multibranch Pipeline")).toBeVisible();
    });
    test("TC_01.005.04 | Verify Multibranch Pipeline type can be selected", async ({ page }) => {
    ...
    test("TC_01.005.05 | Verify OK button requires valid name and item type", async ({ page }) => {
        const projectName = generateProjectName();
        await openNewItemPage(page);
    ...
    test("TC_01.005.06 | Verify user can return to Dashboard without creating item", async ({ page }) => {
    ...
    test("TC_01.005.07 | Verify redirection to configuration page after creating Multibranch Pipeline", async ({ page }) => {
        const projectName = generateProjectName();
        await openNewItemPage(page);
        await page.locator(newItemLocators.itemNameInput).fill(projectName);
...
//# sourceMappingURL=tl-multibranchPipeline.spec.js.map
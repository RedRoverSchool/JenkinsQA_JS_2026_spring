...
test.describe("US_01.005 | New Item > Create Multibranch Pipeline", () => {
    test("TC_01.005.01 | Verify Multibranch Pipeline Creation", async ({ page }) => {
    ...
    for (const invalidChar of jenkinsData.invalidCharacters) {
        test(`TC_01.005.02 | Create Multibranch Pipeline with invalid credentials ${invalidChar}`, async ({ page }) => {
            await page.locator(".task-link-text", { hasText: "New Item" }).click();
...
//# sourceMappingURL=am-verifyMultibranchPipelineCreation.spec.js.map
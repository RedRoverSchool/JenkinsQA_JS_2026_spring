...
test.describe("US_09.001 | Build History > Core Build History Display", () => {
    test("TC_09.001.02 | Verify Build History option is accessible from Dashboard", async ({ page }) => {
        await page.getByRole("link", { name: "Build History" }).click();
        await expect(page).toHaveURL(/builds/);
    });
    test("TC_09.001.03 | Verify Build History displays list of builds", async ({ page }) => {
    ...
    test("TC_09.001.05 | Verify successful build entry displays success status icon", async ({ page }) => {
    ...
    test("TC_09.001.06 | Verify build number links to Build Summary page", async ({ page }) => {
...
test.describe("US_09.002 | Build History > Sorting", () => {
    test("TC_09.002.01 | Verify columns are sortable", async ({ page }) => {
        await createFreestyleProject(page);
        await createBuilds(page, 1);
        await createFreestyleProject(page);
        await createBuilds(page, 1);
...
//# sourceMappingURL=tl-buildHistory.spec.js.map
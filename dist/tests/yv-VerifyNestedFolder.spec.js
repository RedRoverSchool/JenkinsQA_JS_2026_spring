...
test.describe("US_01.002 | New Item > Folder", () => {
    test("TC_01.002.21 | Verify nested folder creation", async ({ page }) => {
    ...
    test("TC_01.002.22 | Verify that error message is displayed", async ({ page }) => {
        await page.getByRole('link', { name: 'New Item' }).click();
        await page.getByRole('radio', { name: 'Folder Creates a container' }).click();
        await expect(page.locator('#itemname-required')).toBeVisible();
    });
});
//# sourceMappingURL=yv-VerifyNestedFolder.spec.js.map